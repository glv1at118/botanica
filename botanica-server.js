...
app.post("/login", function (req, res) {
    ...
    const uri = "mongodb+srv://admin:admin@botanica-database-hli2x.mongodb.net/test?retryWrites=true&w=majority";
    ...
    client.connect(err => {
        if (err) {
            ...
            saves.find({ "userCredentials": { "userName": clientName, "passWord": clientPassword } }).toArray(function (err, docs) {
                if (err) {
                    res.send(err);
                    client.close();
                } else {
...
app.get("/loadSave", function (req, res) {
    ...
    client.connect(err => {
        if (err) {
            ...
            saves.find({ "userCredentials": { "userName": clientNameTmp, "passWord": clientPasswordTmp } }).toArray(function (err, docs) {
                if (err) {
                    res.send(err);
                    client.close();
    ...
    try {
        client.connect(err => {
            if (err) {
                ...
                saves.findOneAndUpdate({ "userCredentials": { "userName": clientNameTmp, "passWord": clientPasswordTmp } }, { $set: { "userSave": uploadedData } });
            }
        });
    } catch (err) {
